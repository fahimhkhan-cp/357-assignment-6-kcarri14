...
int parsing(char *line, Info *information){
    ...
    information->Employment_Firms_Hispanic_Owned = atoll(token);
     ...
     token = strtok(NULL, ",");
    if (!token) return -1;
    information->Employment_Firms_Native_Hawaiian_Owned = atof(token);

     token = strtok(NULL, ",");
    if (!token) return -1;
    information->Employment_Firms_Total = atof(token);

     token = strtok(NULL, ",");
    if (!token) return -1;
    information->Employment_Firms_Women_Owned = atof(token);
